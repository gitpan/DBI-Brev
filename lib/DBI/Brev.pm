...
DBI::Brev - Brevity is the soul of wit. 
...
DBI::Brev provides framework for using DBI in more convenient & laconic way:
...
6) sql_exec executes multiple statements in single transaction using  SQL::SplitStatement::split_with_placeholders
...
If you want call it different name you can use environment variable DBI_CONF or call
 ...
 DBI::Brev::load_config($config_file)
 
Before first invocation of

  db_use($db_alias);
  
See DBI-Brev.t for example. It creates config file on the fly and then use load_config and then db_use.
